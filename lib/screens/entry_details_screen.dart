...
class _EntryDetailsScreenState extends State<EntryDetailsScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildEntryHeader() {
    final formattedDate = DateFormat('EEEE, MMMM d, yyyy').format(widget.entry.date);
    final formattedTime = DateFormat('h:mm a').format(widget.entry.date);
              ...
              children: [
                ...
                if (_transcription == null || _transcription!.isEmpty)
                  ElevatedButton.icon(
                    ...
                    label: Text(_isTranscribing ? 'Processing...' : 'Transcribe'),
                  ),
              ],
            ),
